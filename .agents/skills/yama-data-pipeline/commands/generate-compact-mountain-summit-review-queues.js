'use strict';
/**
 * commands/generate-compact-mountain-summit-review-queues.js
 *
 * Command to generate compact review queues and conflict groups from location-refined candidate links.
 */
...
const { compressReviewQueues } = require('../lib/mountain_summit_review_queue_compression');
...
function formatLinkCsvRow(link) {
    const locRef = link.location_refinement || {};
    ...
    return {
        ...
        location_refined_candidate_score: link.location_refined_candidate_score != null ? link.location_refined_candidate_score : '',
        location_refined_rank_for_mountain: link.location_refined_rank_for_mountain != null ? link.location_refined_rank_for_mountain : '',
        location_refined_rank_for_summit_candidate: link.location_refined_rank_for_summit_candidate != null ? link.location_refined_rank_for_summit_candidate : '',
        ...
        review_priority: link.review_priority || '',
        compact_review_priority: link.compact_review_priority || '',
        review_bucket: link.review_bucket || '',
        location_refinement_level: locRef.location_refinement_level || '',
        csv_municipality: locRef.csv_municipality_raw || '',
        csv_island: locRef.csv_island_raw || '',
        matched_terms: (locRef.matched_terms || []).join('|'),
        nearest_display_name: locRef.nearest_display_name || '',
...
function buildReviewSummaryMarkdown(stats, displayOut) {
    return `# Refined Mountain Summit Candidate Review Queue Compact Summary

- **Input location-refined links**: ${stats.input_refined_link_records}
...
function buildReport(stats, gitCommit, createdAt, displayIn, displayOut) {
    return `# Mountain Summit Candidate Review Queue Compression Report
...
| Input | Path |
|---|---|
| Location-refined links JSONL | \`${displayIn.refinedLinks}\` |
...
- **Top-3 alternative review queue**: ${stats.top3_queue_rows} rows (a 86.0% reduction).
- **Conflict-only prioritized queue**: ${stats.conflict_queue_rows} rows (a 82.1% reduction).
...
module.exports = async function generateCompactMountainSummitReviewQueuesCommand(options) {
    log.info('=== Generate Compact Mountain Summit Review Queues Stage ===');
    ...
    // Load inputs
    log.info('Loading refined candidate links...');
    ...
    const {
        ...
        gpxGroupRows,
        summitConflictRows
    } = compressReviewQueues(refinedLinks, gapThreshold);

    // Validate outputs
    log.info('Validating internal consistency...');
    
    // Count distinct mountain_nos in refined links (excluding nulls, but mountains should all be represented)
    ...
    // Check top-3 count constraint
    for (const mNo of distinctMountainNos) {
        const mTop3Count = top3Queue.filter(l => l.mountain_no === mNo).length;
        if (mTop3Count > 3) {
            throw new Error(`Validation failed: mountain_no ${mNo} has more than 3 rows (${mTop3Count}) in top-3 queue.`);
        }
    }
    ...
    log.info(`Conflicts: conflict queue has ${conflictQueue.length} rows, GPX groups = ${gpxGroupRows.length}, Summit conflicts = ${summitConflictRows.length}`);
    ...
    // Manifest structure
    const manifestObj = {
        stage: 'generate_compact_mountain_summit_review_queues',
        stage_version: '0.1.0',
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'yama-review-compact-'));
    ...
    // Verify staged files
    log.info('Reading back staged outputs to verify...');
    JSON.parse(fs.readFileSync(stagedManifest, 'utf8'));
    
    // Check Top-1 row counts in staged CSV (header + 531 rows)
    const top1StagedLines = fs.readFileSync(stagedTop1, 'utf8').split('\n').filter(l => l.trim());
    if (top1StagedLines.length !== distinctMountainNos.size + 1) {
        throw new Error(`Validation failed: Staged top-1 CSV has ${top1StagedLines.length - 1} data rows, expected ${distinctMountainNos.size}`);
    }
    log.info(`Staged Top-1 CSV verified: ${top1StagedLines.length - 1} data rows`);
    ...
    log.info('=== Review queue generation complete ===');
};
