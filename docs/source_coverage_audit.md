...
| `csv/` | `.csv` | Intermediate / Operational Input | Python script (missing) | yama-data-pipeline | `data/02_intermediate/activity_logs/csv_extracted/` | derived only (provisional) | Pending | High | Yes | Extracted from Excel. Needs decision on manual edits and missing script. |
...
| `.agents/skills/yama-data-pipeline/test/fixtures/` | `.gpx`, `.csv` | Test Fixtures | Humans/Agents | Unit/Integration Tests | *Stays in test/fixtures* | intentionally discarded (from migration) | N/A | None | No | Skill-specific test data, not project dataset |
