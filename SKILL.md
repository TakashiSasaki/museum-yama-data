...
#### 4n. `assign-mountain-summits-gemini-grounded` (Portable)

Proposes exactly one coordinate assignment per CSV-derived mountain record using Gemini geographic grounding anchors and nearby GPX summit candidates.

- Requires `--mountains`, `--summit-candidates`, `--grounding-reference`, `--municipality-lookup`, `--municipality-stability`, `--municipality-adjacency`, `--out`, `--candidate-support-links`, `--pruned-log`, `--manifest`, `--review-dir`, and `--report`.
- Computes spatial matching with distance tiers, scoring calculations, expected-to-candidate municipality compatibilities (including land adjacency checks), and elevation discrepancies.
- All-or-nothing atomicity check: validates outputs in staging before moving them to final destinations; fails if any final target path already exists to enforce the non-overwrite policy.
- Outputs generated by this command are for review planning only and do not replace Stage 25 review queues. They must not be treated as final accepted canonical coordinates.

```sh
node cli.js assign-mountain-summits-gemini-grounded \
  --mountains "data/03_primary/mountains/ehime_mountain_source_rows.json" \
  --summit-candidates "data/03_primary/summit_candidates/2026-05-12/summit_candidates.jsonl" \
...
#### 4o. `assign-mountain-summits-gemini-grounded-balanced` (Portable)

Proposes exactly one coordinate assignment per CSV-derived mountain record using Gemini geographic grounding anchors and nearby GPX summit candidates, with relaxed review rules using boundary-compatible municipalities and activity-title name evidence.

- Requires `--mountains`, `--summit-candidates`, `--grounding-reference`, `--municipality-lookup`, `--municipality-stability`, `--municipality-adjacency`, `--activity-links`, `--out`, `--candidate-support-links`, `--pruned-log`, `--manifest`, `--review-dir`, and `--report`.
- Incorporates Japanese-safe short name safeguards and a strict near-tie conflict detection algorithm.
- Classifies mountains into six review categories including `auto_supported_not_canonical` when strong evidence (e.g. boundary-compatible municipality or activity-title contains mountain name) is present.
- All-or-nothing atomicity check: validates outputs in staging before moving them to final destinations; fails if any final target path already exists to enforce the non-overwrite policy.
- Outputs generated by this command are for review planning only and do not replace Stage 25 review queues. They must not be treated as final accepted canonical coordinates.

```sh
node cli.js assign-mountain-summits-gemini-grounded-balanced \
  --mountains "data/03_primary/mountains/ehime_mountain_source_rows.json" \
  --summit-candidates "data/03_primary/summit_candidates/2026-05-12/summit_candidates.jsonl" \
  --grounding-reference "data/04_feature/mountain_geographic_grounding/2026-06-06/grounding_reference_index.jsonl" \
  --municipality-lookup "data/04_feature/location_reference/municipality_point_lookup/summit_candidates/2026-05-12/summit_candidate_municipality_lookup.jsonl" \
  ...
  --municipality-adjacency "data/04_feature/location_reference/municipality_adjacency/ehime/2026-01-01/municipality_adjacency.json" \
  --activity-links "data/04_feature/activity_linking/gpx_yamap_candidate_links/2026-05-12/title_enriched_candidate_links.jsonl" \
  --out "data/04_feature/mountain_summit_assignments/2026-06-07_gemini_grounded_balanced_summit_assignment/proposed_summit_assignments.jsonl" \
  --candidate-support-links "data/04_feature/mountain_summit_assignments/2026-06-07_gemini_grounded_balanced_summit_assignment/candidate_support_links.jsonl" \
  --pruned-log "data/04_feature/mountain_summit_assignments/2026-06-07_gemini_grounded_balanced_summit_assignment/pruned_candidate_log.jsonl" \
  ...
  --review-dir "data/08_reporting/mountain_summit_assignment_review/2026-06-07_gemini_grounded_balanced_summit_assignment" \
