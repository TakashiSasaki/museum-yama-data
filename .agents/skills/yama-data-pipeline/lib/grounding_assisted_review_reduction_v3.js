...
function processMountain(mountainNo, mountainData, candidates, groundingRef, stage21LinksByMountain, stage21QueueMap) {
    ...
    if (stage21TopId) {
        s21CandidateInS23 = candidates.find(c => c.summit_candidate_id === stage21TopId);
        if (!s21CandidateInS23) {
            // Check by distance tolerance
            const s21Lat = parseFloat(stage21TopCandidate.candidate_lat);
            const s21Lon = parseFloat(stage21TopCandidate.candidate_lon);
            for(const c of candidates) {
                if (c.candidate_lat && c.candidate_lon && s21Lat && s21Lon) {
                    const dist = haversineDistance(c.candidate_lat, c.candidate_lon, s21Lat, s21Lon);
    ...
    if (s21CandidateInS23 && topCandidate && s21CandidateInS23.summit_candidate_id === topCandidate.summit_candidate_id) {
        stage21Agree = true;
    }
    ...
    if (isMarker) {
    ...
    } else if (stage21Support && !s21CandidateInS23) {
        reviewClass = 'review_required_stage21_supported_candidate_missing';
        reasonCodes.push('stage21_supported_candidate_missing');
    } else if (stage21Support && !stage21Agree) {
        reviewClass = 'review_required_stage21_stage23_disagreement';
        ...
        reviewClass = 'review_required_coordinate_conflict';
        ...
        if (closeAlternatives) {
            ...
            reviewClass = 'review_deferred_stage21_supported_top_agrees';
            activeReview = false;
