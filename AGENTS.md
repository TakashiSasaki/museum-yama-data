...
- `csv/`: **Legacy Operational Input**. Contains `.csv` files extracted from four sheets in `data/01_raw/provider_received/yoshitomi/2026-05-18/えひめの山.xlsx` by a historical Python script. They are direct script-generated Excel-derived legacy CSV extracts. They were not manually edited or post-processed after extraction, according to user-provided provenance. They are legacy operational inputs / derived extracts, not independent primary source data. Full reproducibility still requires locating/reconstructing and validating the extraction script or equivalent extraction logic.
- `processed/`: **Legacy Processed-Marker Archive / Retained Source Snapshot Archive**. Files were moved here after old workflow handling to mark them as already processed. Do not describe `processed/` as a clean future raw-data layout. `data/01_raw/provider_received/yoshitomi/2026-05-18/えひめの山.xlsx` and `data/01_raw/provider_received/yoshitomi/2026-05-12/GPXファイル.zip` are retained source snapshots. `data/01_raw/provider_received/yoshitomi/2026-05-18/えひめの山.xlsx` is the primary source workbook for CSV-derived activity records.
