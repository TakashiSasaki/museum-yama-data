...
| `reverse_geocoding/` | `.json` | Snapshot/Cache | reverse_geocode_points.js | extract_address_from_raw.js | `data/01_raw/reverse_geocoding/` | preserved as raw snapshot | Pending | Medium | Yes | Reverse geocoding result cache for municipality-level location enrichment. Needs decision because schema contract and reuse logic are unconfirmed. Derived outputs will go to `data/03_primary/municipalities/` and `data/04_feature/location_enrichment/`. |
