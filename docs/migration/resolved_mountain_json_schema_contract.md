...
*   **Authoritative Primary Key**: The future output must use **`mountain_no`** as the unified effective primary key.
    ...
    *   For rows with a blank CSV `No`, `mountain_no` is derived from the physical CSV row number (expected `503..532`).
    *   `mountain_no` must be a unique, non-null integer.
    *   The expected key set is `1..501` plus `503..532` (with `502` absent/reserved).
...
| source row index | `source_row_no`, `mountain_no` (if blank `No`) | `provenance_fields` / `required_authoritative_fields` | Sourced from CSV physical row number. Used for provisional IDs `503..532`. |
| blank/null `No` record | Included as provisional records | `required_authoritative_fields` | 30 legacy records included, mapped to `mountain_no` `503..532`. |
