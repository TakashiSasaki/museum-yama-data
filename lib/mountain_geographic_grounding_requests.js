...
function generateGroundingRequests({
    ...
    selectedMountains.forEach(({ mountain, links, reasons, top1Csv }) => {
        ...
        const hashContext = `${mNoStr}:${mountain.mountain_name}`;
  ...
  Stability level: ${c.location_stability_level || 'unknown'}`;
        ...
        const prompt_text = `You are a geographic grounding agent. Your task is to verify and resolve the correct geographic location of mountain "${mountain.mountain_name}" (No. ${mNoStr}) in Ehime Prefecture, Japan.
...
1. The true summit coordinates of "${mountain.mountain_name}".
...
Your output must be JSON ONLY matching the requested schema. Do not guess coordinates when evidence is insufficient.`;
        ...
        const requestPacket = {
            ...
            external_agent_task: {
                instruction: `Resolve correct geographic coordinates for mountain "${mountain.mountain_name}" (#${mNoStr}).`,
                prompt_text
            },
...
## Target mountain
- **Mountain No**: ${mNoStr}
- **Mountain Name**: ${mountain.mountain_name}
...
`;
    ...
    const indexPaddedLines = selectedMountains.map(({ mountain }) => {
        const mNoPadded = String(mountain.mountain_no).padStart(6, '0');
        return `- [mountain_${mNoPadded}](mountain_${mNoPadded}.md): ${mountain.mountain_name}`;
    }).join('\n');
