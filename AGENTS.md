...
- `csv/`: **Legacy Operational Input**. Contains `.csv` files extracted from four sheets in `data/01_raw/provider_received/yoshitomi/1980-01-01/えひめの山.xlsx` by a historical Python script. They are direct script-generated Excel-derived legacy CSV extracts. They were not manually edited or post-processed after extraction, according to user-provided provenance. They are legacy operational inputs / derived extracts, not independent primary source data. Full reproducibility still requires locating/reconstructing and validating the extraction script or equivalent extraction logic.
- `processed/`: **Legacy Processed-Marker Archive / Retained Source Snapshot Archive**. Files were moved here after old workflow handling to mark them as already processed. Do not describe `processed/` as a clean future raw-data layout. `data/01_raw/provider_received/yoshitomi/1980-01-01/えひめの山.xlsx` and `data/01_raw/provider_received/yoshitomi/2026-05-12/GPXファイル.zip` are retained source snapshots. `data/01_raw/provider_received/yoshitomi/1980-01-01/えひめの山.xlsx` is the primary source workbook for CSV-derived activity records.
...
- **Authoritative Primary Key:** Use `mountain_no` (sourced from the CSV `No` column) as the authoritative primary key for resolved mountain records. Do not use mountain name, municipality, coordinates, or row index as the primary key. The authoritative key set is strictly 501 records (`1..501`); records with blank `"No"` must be excluded.
