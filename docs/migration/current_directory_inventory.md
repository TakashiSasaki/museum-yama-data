...
| `csv/` | Legacy Input | retained legacy artifact | Must be preserved | Input | protected; no movement | Source coverage audit, Source-to-target mapping audit | Excel-derived legacy CSV extracts, direct script-generated outputs, no manual post-processing. Must not be physically moved yet, nor normalized or regenerated. |
