...
- **Authoritative Primary Key:** Use `mountain_no` (sourced from the CSV `No` column) as the authoritative primary key for resolved mountain records. Do not use mountain name, municipality, coordinates, or row index as the primary key.
