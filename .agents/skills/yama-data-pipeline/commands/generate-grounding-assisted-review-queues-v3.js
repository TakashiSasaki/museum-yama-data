...
const { processMountain } = require('../lib/grounding_assisted_review_reduction_v3');
...
module.exports = async function (options) {
    const candidateLinksPath = options.stage23CandidateLinks || options['stage23-candidate-links'];
    ...
    const diagnosticsPath = options.diagnostics;
    ...
    // Output lists
    const reviewDeferredMountains = mountainResults.filter(m => m.review_classification.startsWith('review_deferred'));
    ...
    const conflictMountains = mountainResults.filter(m => m.review_classification === 'review_required_conflict' || m.review_classification === 'review_required_coordinate_conflict' || m.review_classification === 'review_required_grounding_coordinate_conflict');
    const noCandidateMountains = mountainResults.filter(m => m.review_classification === 'review_required_no_candidate');
    const stage21Disagreements = mountainResults.filter(m => m.review_classification === 'review_required_stage21_stage23_disagreement');
    const stage21Missing = mountainResults.filter(m => m.review_classification === 'review_required_stage21_supported_candidate_missing');

    const immediateReviewRequiredMountains = mountainResults.filter(m =>
        m.active_review_required &&
        !m.review_classification.startsWith('review_deferred') &&
        m.review_classification !== 'auto_supported_strict_grounding_match' &&
        m.review_classification !== 'map_check_recommended' &&
        m.review_classification !== 'review_required_no_candidate' &&
        m.review_classification !== 'review_required_conflict' &&
        m.review_classification !== 'review_required_coordinate_conflict' &&
        m.review_classification !== 'review_required_grounding_coordinate_conflict' &&
        m.review_classification !== 'review_required_stage21_stage23_disagreement' &&
        m.review_classification !== 'review_required_stage21_supported_candidate_missing'
    );

    // Filter candidates for candidate-level outputs
    const activeCandidateIds = new Set();
    [...immediateReviewRequiredMountains, ...conflictMountains, ...noCandidateMountains, ...stage21Disagreements, ...stage21Missing].forEach(m => activeCandidateIds.add(m.mountain_no));
    ...
    const categories = [immediateReviewRequiredMountains, reviewDeferredMountains, autoSupportedMountains, mapCheckMountains, conflictMountains, noCandidateMountains, stage21Disagreements, stage21Missing];
    ...
    fs.writeFileSync(path.join(outDir, 'stage21_supported_candidate_missing_from_stage23.csv'), formatCsv(stage21Missing, mCols));
    fs.writeFileSync(path.join(outDir, 'auto_supported_mountains.csv'), formatCsv(autoSupportedMountains, mCols));
    ...
    // Manifest
    const totalActive = immediateReviewRequiredMountains.length + conflictMountains.length + noCandidateMountains.length + stage21Disagreements.length + stage21Missing.length;
    const manifest = {
        summary: {
            total_mountains: 531,
            ...
            stage26_auto_supported_mountains: autoSupportedMountains.length,
            stage26_review_deferred_mountains: reviewDeferredMountains.length,
            stage26_map_check_recommended_mountains: mapCheckMountains.length,
            stage26_immediate_review_required_mountains: totalActive,
            stage26_conflict_mountains: conflictMountains.length,
            stage26_no_candidate_mountains: noCandidateMountains.length,
            stage26_stage21_stage23_disagreements: stage21Disagreements.length,
            stage26_stage21_candidate_missing: stage21Missing.length,
            coverage_gap: 0,
    ...
    // Summary MD
    const summaryMd = `# Stage 26 / v3 Review Queue Summary
- Total Mountains: 531
...
- Stage 26 Active Review Mountains: ${totalActive}
`;
    fs.writeFileSync(path.join(outDir, 'summary.md'), summaryMd);

    log.info(`Stage 26 completion: ${totalActive} active review mountains generated.`);
};
