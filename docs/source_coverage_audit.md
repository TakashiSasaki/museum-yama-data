...
| `gpx/annotated/` | `.gpx` | Artifact | yama-data-pipeline (annotate) | Web App | TBD | needs decision | Pending | Low (regenerable) | Yes | Legacy experimental derived artifacts produced by the legacy `annotate` command. Preserved as legacy evidence; unvalidated derived artifact; not authoritative. Existing algorithm output and summit names are not trusted. Future pipeline output should generate summit-candidate waypoints separately; summit identity/name resolution is a separate downstream step. |
| `gpx/merged-by-year/` | `.gpx` | Artifact | yama-data-pipeline (merge) | Google My Maps | TBD | needs decision | Pending | Low (regenerable) | Yes | Legacy yearly merged GPX overview/reporting artifacts for whole-dataset browsing and Google My Maps import. Must be preserved until the merge pipeline and validation rules are formalized. |
...
| `reverse_geocoding/` | `.json` | Snapshot/Cache | reverse_geocode_points.js | extract_address_from_raw.js | `data/01_raw/reverse_geocoding/` | preserved as raw snapshot | Pending | Medium | No | Reverse geocoding result cache for municipality-level location enrichment. Schema contract and reuse logic still require formalization. Derived outputs will go to `data/03_primary/municipalities/` and `data/04_feature/location_enrichment/`. |
