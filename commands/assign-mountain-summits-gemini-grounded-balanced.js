...
const { performAssignment } = require('../lib/gemini_grounded_balanced_summit_assignment');
...
module.exports = async function (options) {
    log.info('Starting balanced Gemini-grounded mountain summit assignment...');
    ...
    const stagingFeatureDir = path.join(path.dirname(finalOut), '.staging_balanced');
    const stagingReviewDir = path.join(reviewDir, '.staging_balanced');
    ...
    const stagedReport = path.join(stagingFeatureDir, 'mountain_summit_assignment_gemini_grounded_balanced_report.md');
    ...
    // Boundary-compatible auto supported counts
    const boundaryCompatibleAutoCount = proposedAssignments.filter(a => 
        a.review_category === 'auto_supported_not_canonical' && 
        a.evidence.municipality.compatibility === 'municipality_boundary_compatible'
    ).length;

    // Activity-title supported counts
    const activityTitleSupportedCount = proposedAssignments.filter(a => 
        a.review_category === 'auto_supported_not_canonical' && 
        a.evidence.name.name_evidence_sources.includes('yamap_activity_title')
    ).length;
    ...
    if (options.activityLinks && fs.existsSync(options.activityLinks)) {
        inputHashes.activity_links = getSha256(options.activityLinks, true);
    }
    ...
    const summaryObject = {
        ...
        boundary_compatible_auto_supported_count: boundaryCompatibleAutoCount,
        activity_title_supported_count: activityTitleSupportedCount,
        by_review_category: {
    ...
    const manifestJson = {
        ...
        method_id: 'gemini_grounded_balanced_summit_assignment',
        run_id: '2026-06-07_gemini_grounded_balanced_summit_assignment',
    ...
    // Staged summary.md
    const summaryMdContent = `# Gemini-Grounded Balanced Mountain Summit Assignment Review Summary
...
* **Boundary-Compatible Auto Supported**: ${boundaryCompatibleAutoCount}
* **Activity Title Supported**: ${activityTitleSupportedCount}
...
`;
    ...
    // Staged report.md
    const reportMdContent = `# Mountain Summit Assignment (Gemini-Grounded Balanced) Validation Report
...
* Activity links: \`${options.activityLinks || '(none)'}\`
...
`;
    ...
    log.info('All balanced assignments completed and validated successfully.');
};
