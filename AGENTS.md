...
- **Authoritative Primary Key & Source Scope:** Use `mountain_no` as the unified effective primary key. All 531 rows in `愛媛県の山.csv` are in scope. For `愛媛県の山.csv`, Excel sheet extraction preserves the source sheet values. A separate mountain source acceptance/normalization step fills blank `No` values using the 1-based physical CSV row number and then validates uniqueness of the effective `mountain_no` (`1..501` and `503..532`). Duplicate effective `mountain_no` values are a fatal acceptance error. Downstream mountain processing must use the accepted/normalized dataset, not raw extracted CSV rows with blank effective IDs. CSV coordinates (`GPS` column) in provisional rows must be preserved as raw coordinate evidence (`gps_raw`). Do not use mountain name, municipality, or coordinates as the primary key.
