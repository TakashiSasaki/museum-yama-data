...
function hasSevereWarnings(link) {
    ...
    if (link.location_stability_bucket === 'municipality_incompatible_strong') {
        return true;
    }
    return false;
}

function compressStabilityReviewQueues(links, gapThreshold = 0.03) {
    ...
    // 2. Identify top-ranked counts per candidate using refined ranks
    ...
    for (const link of links) {
        ...
        const rank = link.location_stability_refined_rank_for_mountain;
        const score = link.location_stability_refined_candidate_score;
        ...
        const r1Link = mtLinks.find(l => l.location_stability_refined_rank_for_mountain === 1);
        const r2Link = mtLinks.find(l => l.location_stability_refined_rank_for_mountain === 2);

        if (r1Link) {
            topScore = r1Link.location_stability_refined_candidate_score;
            gapFromTop = topScore - score;
        }

        if (rank && mtLinks.length > 0) {
            const nextLink = mtLinks.find(l => l.location_stability_refined_rank_for_mountain === rank + 1);
            if (nextLink) {
                gapToNext = Number((score - nextLink.location_stability_refined_candidate_score).toFixed(6));
            }
        }

        const scoreGapRank1To2 = (r1Link && r2Link) ? Number((r1Link.location_stability_refined_candidate_score - r2Link.location_stability_refined_candidate_score).toFixed(6)) : null;

        // Mutual top1 and top3
        const isMutualTop1 = candId ? (rank === 1 && link.location_stability_refined_rank_for_summit_candidate === 1) : false;
        const isMutualTop3 = candId ? (rank <= 3 && link.location_stability_refined_rank_for_summit_candidate <= 3) : false;
        ...
        const hasLocationWarning = (link.location_stability_bucket === 'boundary_plausible' ||
                                    link.location_stability_bucket === 'adjacent_but_deep_inside' ||
                                    link.location_stability_bucket === 'municipality_incompatible_strong');
        ...
        if (!candId) {
            ...
            compactReasonCodes.push(link.location_stability_bucket);
        } else if (link.location_stability_review_priority === 'deprioritized' && !isTop3) {
    ...
    for (const [mtNo, mtLinks] of linksByMountain) {
        const r1 = enrichedLinks.find(l => l.mountain_no === mtNo && l.location_stability_refined_rank_for_mountain === 1);
        if (r1) {
            top1Queue.push(r1);
        } else {
    ...
    for (const [mtNo, mtLinks] of linksByMountain) {
        ...
        const top3Candidates = mtEnriched.filter(l => l.location_stability_refined_rank_for_mountain !== null && l.location_stability_refined_rank_for_mountain <= 3);
    ...
    top3Queue.sort((a, b) => {
        ...
        const rankA = a.location_stability_refined_rank_for_mountain || 999;
        const rankB = b.location_stability_refined_rank_for_mountain || 999;
        return rankA - rankB;
    });
    ...
    for (const [candId, candLinks] of linksByCandidate) {
        ...
        const top1Mountains = candLinks.filter(l => l.location_stability_refined_rank_for_mountain === 1);
        const top3Mountains = candLinks.filter(l => l.location_stability_refined_rank_for_mountain !== null && l.location_stability_refined_rank_for_mountain <= 3);
        ...
        const scores = candLinks.map(l => l.location_stability_refined_candidate_score);
