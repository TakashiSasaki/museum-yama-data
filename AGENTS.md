...
- The `csv/` directory is the current legacy operational input for activity metadata used by the existing pipeline. These CSV files are direct script-generated extracts from 6 sheets in `data/01_raw/as_received/2026-05-18/えひめの山.xlsx`. For provenance purposes, `data/01_raw/as_received/2026-05-18/えひめの山.xlsx` is the retained source snapshot / primary source workbook. The CSV files are Excel-derived legacy CSV extracts; they were not manually edited or post-processed according to user-provided provenance. While the `extract-excel-sheets` subcommand in the `yama-data-pipeline` skill can perform extraction, full reproducibility and migration remain blocked pending validation of exact historical extraction behavior and sheet mapping.
