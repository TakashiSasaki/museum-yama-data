...
- `data/01_raw/yamap_markdown/`: **Raw YAMAP Markdown Snapshots**. Contains Markdown snapshots fetched from YAMAP pages via the `fetch-yamap-data` skill. Extra YAMAP metadata not referenced by GPX is acceptable and must not be deleted automatically. Formerly located at legacy `yamap/`.
